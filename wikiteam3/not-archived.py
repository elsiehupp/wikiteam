...
def main():
    ...
    with ssl.SSLContext(
        ...
        with urllib.request.urlopen(doneurl, context=ssl_context) as request_context:

            wikiurls = re.findall(
                r'(?im)<a href=\\"(http[^>]+?)\\" rel=\\"nofollow\\">[^<]+?</a> dumped with',
                str(request_context.read()),
            )
            ...
            while True:
                ...
                request_context = urllib.request.urlopen(url, context=ssl_context)
                raw = request_context.read()
                match = re.findall(
