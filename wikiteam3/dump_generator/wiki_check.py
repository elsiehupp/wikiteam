...
def get_wiki_engine(url: str) -> str:
    ...
    with requests.Session() as session:
        ...
        with session.post(url=url, timeout=30) as post_response:
            if post_response.status_code == 405 or post_response.text == "":
                post_response = session.get(url=url, timeout=120)
        ...
        if re.search(
            ...
            r'(?im)(<meta http-equiv="powered by" content="MojoMojo)', result
        ):
            wikiengine = "MojoMojo"
        elif re.search(
            ...
            r'(?im)(powered by <a href="[^"]*\bzwiki.org(/[^"]*)?">)', result
        ):
            wikiengine = "Zwiki"
        # WakkaWiki forks
        elif re.search(
            ...
            r'(?im)(Fonctionne avec <a href="http://www\.wikini\.net)', result
        ):
            wikiengine = "WikiNi"
        elif re.search(
            r'(?im)(Powered by <a href="[^"]*CitiWiki">CitiWiki</a>)', result
        ):
            wikiengine = "CitiWiki"
        elif re.search(
            r'(?im)(Powered by <a href="http://wackowiki\.com/|title="WackoWiki")',
            result,
        ):
            wikiengine = "WackoWiki"
        elif re.search(
            r'(?im)(Powered by <a href="http://www\.wakkawiki\.com)', result
        ):
        ...
        # Custom wikis used by wiki farms
            ...
            r'(?im)(var wikispaces_page|<div class="WikispacesContent)', result
        ):
